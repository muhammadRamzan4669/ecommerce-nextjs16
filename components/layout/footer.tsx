    ...
    <footer className="bg-[#F0F0F0] dark:bg-[#1a1a1a] pt-[140px] lg:pt-[188px]">
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-[100px]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 lg:gap-[110px] pb-12 lg:pb-[50px]">
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map((social) => (
                <Link
                  ...
                  className="w-7 h-7 rounded-full bg-white dark:bg-black border border-black/10 dark:border-white/10 flex items-center justify-center hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors"
          ...
          {/* Company */}
          <div>
            <h3 className="font-medium text-base tracking-[3px] uppercase mb-6">Company</h3>
            <ul className="space-y-4">
              {footerLinks.company.map((link) => (
                <li key={link.name}>
                  <Link 
                    href={link.href}
                    className="text-base text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white transition-colors"
                  >
                    {link.name}
                  </Link>
                </li>
          ...
          {/* Help */}
          <div>
            <h3 className="font-medium text-base tracking-[3px] uppercase mb-6">Help</h3>
            <ul className="space-y-4">
          ...
          {/* FAQ */}
          <div>
            <h3 className="font-medium text-base tracking-[3px] uppercase mb-6">FAQ</h3>
            <ul className="space-y-4">
          ...
          {/* Resources */}
          <div>
            <h3 className="font-medium text-base tracking-[3px] uppercase mb-6">Resources</h3>
            <ul className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            {paymentMethods.map((method) => (
              ...
              >
                ...
                {method === "Apple Pay" && <ApplePayIcon />}
                {method === "Google Pay" && <GooglePayIcon />}
              </div>
            ))}
          </div>
        </div>
...
function FacebookIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 8 14" fill="currentColor">
